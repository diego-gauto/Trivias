...
const CancelSuscription = () => {
  ...
  const getTimeReward = async (props: any) => {
    ...
    if (monthFilter.length > 0) {
      if (monthRewardCompleted.length === 0) {
        progressMonth =
          dashArray -
          ((monthProgress - 0) / (monthFilter[0].month - 0)) * dashArray;
      } else {
        progressMonth =
          dashArray -
  ...
  const getNextRewards = async (data: any) => {
    ...
    const { monthPercentage, nextCourseCertificate, reward, user } = data;
    ...
    if (pointsFilter.length > 0) {
      if (pointRewardCompleted.length === 0) {
        progressPoints =
          dashArray -
          ((user.score - 0) / (pointsFilter[0].points - 0)) * dashArray;
      } else {
        progressPoints =
          dashArray -
  ...
  return (
    <CancelSub>
      <div className='m-3'>
        ...
        <p className='my-4'>
          Sabemos que hay momentos en la vida en la que necesitas hacer un
          pausa.{' '}
        </p>
        {/* <br />Por eso, <b>te ofrecemos la opcion de pausar tu suscripción por un mes.</b></p> */}
        <p className='my-4'>
          Quizás no has podido tomar tus clases o no cuentas con el dinero
          suficiente para continuar.{' '}
        </p>
        <p className='my-4'>
          Pero debo decirte que <b>Gonvar+</b> no sólo son clases, también
          tienes recompensas, beneficios y certificados que{' '}
          <b>perderás al cancelar:</b>
        </p>
        ...
        <div className='row w-100 justify-content-around'>
          <div className='col-sm text-center'>
            ...
            <CircleProgress
              progress={data}
              total={totalPointRewards.length}
              color={'#f88114'}
            />
            <p>
              <b className='orange'>{userData.score}</b> Puntos obtenidos
            </p>
            <p>
              <b className='orange'>{totalPointRewards.length}</b> Recompensas
              obtenidas
            </p>
            <p>
              Tu próxima recompensa a los{' '}
              <b className='orange'>
                {pointsRewards ? pointsRewards.points : 0}
              </b>{' '}
              puntos será:
            </p>
            <p>
              <b className='orange'>
                {pointsRewards
                  ? pointsRewards.title
                  : 'Ya tienes todas las recompensas!'}
              </b>
            </p>
          </div>
          <div className='col-sm text-center'>
            <CircleProgress
              ...
              total={monthReward.length}
              color={'#29c784'}
            />
            <p>
              Llevas <b className='green'>{timeLevel}</b> meses inscrita a{' '}
              <b>Gonvar+</b>, por lo cual tienes{' '}
              <b className='green'>
                {monthReward[0]
                  ? monthReward[0].title
                  : 'por lo cual aun no obtienes descuento en productos Gonvar'}
                .
              </b>
            </p>
            <p>
              Además tienes <b className='green'>{timeLevel}</b> boletos para
              nuestra rifa que será en el{' '}
            </p>
            <p className='close'>
              <b className='green'>{getMonthForRaffle()}</b>
            </p>
          </div>
          <div className='col-sm text-center'>
            <CircleProgress
              progress={certificateProgress}
              total={userData.user_certificates.length}
              color={'#1b7beb'}
            />
            <p>
              Haz obtenido{' '}
              <b className='blue'>{userData.user_certificates.length}</b>{' '}
              certificados con FUV{' '}
              <b>
                {'('}folio único verificado{')'},
              </b>{' '}
              y estás a punto de conseguir el certificado del curso{' '}
            </p>
            <div className='close'>
        ...
        <p className='my-4'>
          <b>¡Haz las cuentas!</b>, todo lo que Gonvar te regala te conviene.
        </p>
        <p className='my-4'>
          Si eres una aplicadora que gasta al menos 250 pesos al mes en producto
          de uñas, créeme que <b>te saldrá más económico</b> quedarte a recibir
          todos los regalos, que cancelar y comprar de todas formas producto de
          baja calidad, en otro lado y al mismo precio que el de Gonvar.
        </p>
        <p className='my-4'>
        ...
        <div className='buttons mt-5'>
          <button onClick={goCancel} className='left'>
            Perder mis beneficios
          </button>
