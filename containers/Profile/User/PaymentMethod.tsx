...
const PaymentMethod = ({ data, pm, handleClick, newCard, addPayment }: any) => {
  ...
  const detachPayment = async (card: any) => {
    if (
      (card.default && isSuscribedUser(user.data.level)) ||
      (paymentMethods.length === 1 && isSuscribedUser(user.data.level))
    ) {
      setShow(true);
      return;
    }
  ...
  useEffect(() => {
    ...
    conektaPm(body).then((res) => {
      ...
      const extractedProperties = conektaPaymentMethods.map(
        ({ id, brand, last4, default: boolean }: any) => ({
          id,
          brand,
          last4,
          default: boolean,
  ...
  return (
    <PaymentMethodContainer add={addPayment}>
      ...
      <div className='main-container'>
        <div className='title'>Métodos de pago</div>
        {!deleteLoad ? (
          <>
                    ...
                    <React.Fragment key={'pmUser ' + index}>
                      <div className='card-contain'>
                        <div
                          className='card'
                          onClick={() =>
                            data.payment_method != pm.id && updateUserCard(pm)
                          }
                        >
                          <CardIconResp>
                            {pm.brand == 'visa' && (
                          ...
                          <p className='text-card'>
                            Tarjeta de débito |{' '}
                            <span className='last-digits'>Terminación</span>
                            <span className='last-4'> •••• {pm.last4}</span>
                          </p>
                          {pm.default ? (
                            <div className='star'>
                              <AiFillStar />
        ...
        <div className='bottom-contain' onClick={newCard}>
          {!addPayment ? <AiOutlinePlus /> : <AiOutlineMinus />}
          <p>
            Agregar nueva <span>tarjeta de crédito o débito</span>{' '}
          </p>
        </div>
      </div>
      {addPayment && (
        <div className='new-card'>
          <p className='main-title'>
            Nueva <span> tarjeta de crédito o débito</span>
          </p>
