...
import {
  addUserCertificateApi,
  getUserCertificateApi,
  ...
  useEffect(() => {
    ...
    course.seasons.forEach((s: any) => {
      s.lessons.forEach((l: any) => {
        let array = l.progress.filter(
          (x: any) => x.user_id === userData.user_id && x.status === 1,
        );
        if (
          l.users.includes(userData?.user_id) &&
          l.homework === 0 &&
          l.quiz === 0
        ) {
          viewed++;
    ...
    if (course.lessons.length == viewed) {
      ...
      if (userData.level === 2) {
        let course = userData.user_courses.filter(
      ...
      <MainContainer open={open}>
        <div className='course-info'>
          ...
          <div className='level-container'>
            ...
            <Text03 style={{ padding: 0 }} level={course?.difficulty}>
              <span>{course?.difficulty}</span>
            </Text03>
          </div>
        </div>
        {course.with_certificate === 1 && certficate && !responsive1124 && (
        ...
        <div className='course-progress'>
          <p className='title'>
            Tu progreso <br />
            <b>
              {count} de {course.lessons.length}
            </b>{' '}
            <span>lecciones.</span>
          </p>
