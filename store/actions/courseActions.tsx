...
export const getSeason = async (courseId: any) => {
  let courses: any = [];
  const docRef = query(
    collection(db, 'courses', courseId, 'seasons'),
    orderBy('name'),
  );
  const querySnapshot = await getDocs(docRef);
  querySnapshot.forEach((doc) => {
...
export const updateLesson = async (
  ...
  if ('format' in tempLesson) {
    tempLesson.image = await uploadImage(
      tempLesson.format,
      tempLesson.imageReference,
    );
...
export const getWholeCourses = async () => {
  ...
  await Promise.all(
    courses.map(async (course: any) => {
      const docRefSeasons = query(
        collection(db, 'courses', course.id, 'seasons'),
        orderBy('season'),
      );
      ...
      await Promise.all(
        course.seasons.map(async (season: any) => {
          const docRefLesson = query(
            collection(
              db,
              'courses',
              course?.id,
...
export const getWholeCourse = async (courseId: any) => {
  ...
  for (let c = 0; c < seasons.length; c++) {
    const docRefLesson = query(
      collection(db, 'courses', courseId, 'seasons', seasons[c].id, 'lessons'),
      orderBy('number'),
    );
    const querySnapshotLesson = await getDocs(docRefLesson);
    querySnapshotLesson.forEach((lesson: any) => {
...
export const addUserToLesson = async (
  lesson: any,
  courseId: any,
  seasonId: any,
  lessonId: any,
...
export const uploadImageHomework = async (homework: any) => {
  const image = await uploadImageHwk(
    homework.path,
    homework.lessonId,
    homework.userId,
  );
...
export const addHistoryCourse = async (
  course: any,
  userId: any,
  season: any,
  lesson: any,
    ...
    for (let l = 0; l < course.seasons[s].lessons.length; l++) {
      await deleteDoc(
        doc(
          ...
          course.seasons[s].id,
          'lessons',
          course.seasons[s].lessons[l].id,
        ),
      );
      if (l == course.seasons[s].lessons.length - 1) {
        await deleteDoc(
          doc(db, 'courses', course.id, 'seasons', course.seasons[s].id),
        );
      }
    }
...
export const addTeacher = async (professor: any) => {
  ...
  professor.sign = await uploadSignImage(
    professor.sign,
    professor.referenceSign,
  );
  professor.path = await uploadProfessorImage(
    professor.path,
    professor.reference,
  );
...
export const deleteTeacher = async (professor: any) => {
  ...
  const desertRefSign = ref(
    storage,
    `professorSign/${professor.referenceSign}`,
  );
  await deleteObject(desertRef)
    .then(async () => {
      await deleteObject(desertRefSign)
        .then(async () => {
...
export const updateTeacher = async (professor: any, id: any) => {
  ...
  if ('format' in tempProfessor) {
    ...
    tempProfessor.path = await uploadProfessorImage(
      tempProfessor.format,
      tempProfessor.reference,
    );
  ...
  if ('formatSign' in tempProfessor) {
    ...
    tempProfessor.sign = await uploadSignImage(
      tempProfessor.formatSign,
      tempProfessor.referenceSign,
    );
...
const uploadLessonImage = (
  ...
  const storageRef = ref(
    storage,
    `courses/${courseID}/season/${seasonID}/cover/${lessonID}`,
  );
  return new Promise((resolve, reject) => {
...
const uploadLessonHomeWorks = (
  ...
  const storageRef = ref(
    storage,
    `courses/${courseID}/season/${seasonID}/extra/${lessonID}/${title}`,
  );
  return new Promise((resolve, reject) => {
    uploadString(storageRef, lesson, 'data_url').then((snapshot) => {
      getDownloadURL(snapshot.ref).then((downloadURL) => {
...
export const updateLessonHomeWorks = async (
  courseID: number,
  seasonID: number,
  image: any,
  lessonID: number,
  ...
  const url = await uploadLessonHomeWorks(
    courseID,
    seasonID,
    image,
    lessonID,
