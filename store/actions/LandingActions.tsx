...
export const getLandingData = async () => {
  ...
  const featureShowcaseSectionRef = doc(
    db,
    'landingPage',
    'featureShowcaseSection',
  );
  const reseniasCollectionRef = collection(
    db,
    'landingPage',
    'reseniasSection',
    'resenias',
  );
  const experienciasCollectionRef = collection(
    db,
    'landingPage',
    'experienciasUsuario',
    'resenias',
  ...
  const parsedReseniasData = reseniasDocs.docs.map((d) => {
    ...
    return {
      title: nombre,
      imgURL: downloadFileWithStoragePath(imgURL),
      id: d.id,
    };
  });
  ...
  const parsedProductosDestacadosData = productosDestacadosDocs.docs.map(
    (d) => {
      const {
        nombre,
        precio,
...
export const saveProductsData = async (products: Product[]) => {
  const productsCollection = db
    ...
    .doc('productosDestacadosSection')
...
export const saveReviewsData = async (reviewsData: Review[]) => {
  const reviewsCollection = db
    .collection('landingPage')
    .doc('experienciasUsuario')
    .collection('resenias');
  ...
  const fileBgPromises = updatedBg.map(async ({ file, id }) => {
    const result = await uploadFile(
      file!,
      `landing/experiencias/publication/${id}`,
    );
    return reviewsCollection.doc(id).update({
      imgURL: result.metadata.fullPath,
    });
