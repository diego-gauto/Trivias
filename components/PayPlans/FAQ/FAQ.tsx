      ...
      <div className='faq'>
        <div className='bg-white q-container m-3 w-100' onClick={() => verQ(1)}>
          <div className={`q ${views.get(1) && 'open-q'}`}>
            <p className={`mt-3 ms-2 ${views.get(1) && 'open-q'}`}>
              ¿Entregan reconocimiento?
            </p>
            {views.get(1) ? (
              <BsChevronUp className='icon me-3' />
            ) : (
              <BsChevronDown className='icon me-3' />
            )}
          </div>

          {views.get(1) && (
            <div className='border-top mx-3'>
              <p className='py-2 animate__animated animate__fadeIn'>
                ¡Claro!
                <br />
        ...
        <div className='bg-white q-container m-3 w-100' onClick={() => verQ(2)}>
          <div className={`q ${views.get(2) && 'open-q'}`}>
            <p className={`mt-3 ms-2 ${views.get(2) && 'open-q'}`}>
              ¿Qué metodos de pago puedo usar para pagar mi suscripción?
            </p>
            {views.get(2) ? (
              <BsChevronUp className='icon me-3' />
            ) : (
          ...
          {views.get(2) && (
            <div className='border-top mx-3'>
              <p className='py-2 animate__animated animate__fadeIn'>
                ...
                <br />
                Si deseas pagar por mes, puedes hacerlo con cualquier tarjeta de
                crédito o débito.
                <i>(El cobro se realiza de manera automática mes con mes).</i>
                <br />
                Si prefieres pagar en transferencia, depósito en Oxxo o Paypal,
                está disponible la anualidad o el plan cuatrimestral,{' '}
                <i>
                  (Pagando anualidad no se realiza ningún cobro adicional por un
                  año).
                </i>
              </p>
            </div>
        ...
        <div className='bg-white q-container m-3 w-100' onClick={() => verQ(3)}>
          <div className={`q ${views.get(3) && 'open-q'}`}>
            <p className={`mt-3 ms-2 ${views.get(3) && 'open-q'}`}>
        ...
        <div className='bg-white q-container m-3 w-100' onClick={() => verQ(4)}>
          <div className={`q ${views.get(4) && 'open-q'}`}>
            <p className={`mt-3 ms-2 ${views.get(4) && 'open-q'}`}>
          ...
          {views.get(4) && (
            <div className='border-top mx-3 animate__animated animate__fadeIn'>
              <p className='py-2'>
                Si, nuestra plataforma está diseñada para poder utilizarse desde
                cualquier lugar del mundo que cuente con acceso a internet y
                puedes disfrutar desde un celular, tableta electrónica o
                computadora.
              </p>
            </div>
          )}
