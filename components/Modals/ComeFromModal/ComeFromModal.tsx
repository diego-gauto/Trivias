...
const ComeFromModal = (props: IComeFromModal) => {
  ...
  const [errorPhone, setErrorPhone] = useState<PhoneErrorText>(!user.phone_number ? '' : '');
  const [originState, setOriginState] = useState<string>(user.origin_state || '');
  const [errorState, setErrorState] = useState<string>('');
  const [loader, setLoader] = useState<boolean>(false);
  const [isCorrectPhoneNumber, setIsCorrectPhoneNumber] = useState<"yes" | "no">("yes");
  const [isCorrectOriginState, setIsCorrectOriginState] = useState<"yes" | "no">("yes");
  ...
  const generatePhoneNumberInput = () => {
    return (
      <div className="data-container" style={{ paddingTop: '15px' }}>
        <p className="email">
          {!!user.phone_number ? 'Whatsapp' : 'No tenemos tu Whatsapp, por favor ingresa el tuyo'}
        </p>
        <small style={{
          display: 'block',
          fontSize: '14px',
  ...
  const generatePhoneNumberQuestion = () => {
    ...
    return (
      <div className="data-container" style={{ paddingTop: '15px' }}>
        <p>Tu numero de WhatsApp registrado es: <strong>{user.phone_number}</strong></p>
        ...
        <form action="">
          <div style={{
            ...
            <div>
              ...
              />
              <label style={{ paddingLeft: '5px' }} htmlFor="radio-is-correct-phone-number">Si</label>
            </div>
            <div>
              <input
                ...
                onChange={onOptionChange}
              />
              <label style={{ paddingLeft: '5px' }} htmlFor="radio-is-not-correct-phone-number">No, quiero editar mi numero</label>
            </div>
          </div>
        </form>
        {
  ...
  const generateOriginStateInput = () => {
    return <div>
      ...
      >Su estado de procedencia es:</small>
      <SelectInput
        onChange={(e) => {
  ...
  const generateOriginStateQuestion = () => {
    ...
    return (
      <div className="data-container" style={{ paddingTop: '15px' }}>
        <p>Tu estado de origen registrado es: <strong>{user.origin_state}</strong></p>
        <p>¿Es correcto?</p>
        <form action="">
          <div style={{
            display: 'flex',
            ...
            <div>
              <input
                ...
                onChange={onOptionChange}
              />
              <label style={{ paddingLeft: '5px' }} htmlFor="radio-is-correct-origin-state">Si</label>
            </div>
            <div>
              <input
                ...
                onChange={onOptionChange}
              />
              <label style={{ paddingLeft: '5px' }} htmlFor="radio-is-correct-origin-state">No</label>
            </div>
          </div>
        </form>
        {
  ...
  return (
    <Modal show={show}>
      <ComeFromContainer>
          ...
          !user.come_from ?
            <div className='bottom-data'>
              ...
              <div className='select-container'>
                ...
                >
                  <option value="" disabled key={"socials_" + 0}>Seleccione una opción</option>
                  {
                    SOCIALS_ARRAY.map((val: string, index: number) => {
                      return (
                        <option value={val} key={"socials_" + (index + 1)}>{val}</option>
                      )
                    })
                  }
                </SelectInput>
        ...
        {
          phoneNumber.startsWith('52') && !!user.origin_state ? generateOriginStateQuestion() : <></>
        }
        {
          phoneNumber.startsWith('52') && !user.origin_state ? generateOriginStateInput() : <></>
        }
        {
          /*
          <>
