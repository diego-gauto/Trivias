...
const CreateBlog = () => {
  ...
  const removeTheme = (index: number) => {
    ...
    if (
      confirm(
        `¿Desea eliminar subtema ${index + 1}?, esto recorrera los otros subtemas`,
      )
    ) {
            ...
            if (blogsImages.length > 0) {
              blogsImages.forEach((subtpc: any, index: number) => {
                ...
                if (subtpc.image !== '') {
                  ...
                  subtpc.image = '';
                  updateSubTopicImage(tempSubTopicImg, blog.id, subtpc.id).then(
                    (url_subtopic: any) => {
  ...
  const updateFinalProcess = async () => {
    ...
    await Promise.all(
      blog.subTopic.map(async (b: any) => {
        if (b.newImage && b.id) {
          await updateSubTopicImage(b.newImage, blog.id, b.id).then(
            (subUrl) => {
              b.image = subUrl;
            },
          );
    ...
    await updateBlogsApi(blog).then(async (newSubTopicId) => {
      if (subTopicWithNoId.length > 0) {
        await Promise.all(
          subTopicWithNoId.map((subTpc: any, index: number) => {
  ...
  const deleteBlogSql = async () => {
    ...
    if (
      confirm(
        '¿Quieres eliminar este blog?, Esta acción no tiene marcha atrás.',
      )
    ) {
          ...
          <div className='blog-buttons'>
            {canEdit && (
              <button className='add-theme' onClick={addTheme}>
                <p className='theme-text'>Agregar Subtema</p>
              </button>
            )}
            {!processLoader ? (
              <button
                className='create-blog'
                onClick={!blogId ? createNewBlog : editBlog}
              >
            ...
            {blogId && canDelete && (
              <button className='delete-blog' onClick={deleteBlogSql}>
                <p className='theme-text'>Eliminar blog</p>
              </button>
            )}
          </div>
        </div>
        {loader ? (
          <div className='blog-form'>
            <div className='blog-third-part-responsive'>
              ...
              <BlogInputs>
                <label className='blog-label'>Subtítulo</label>
                <input
                  className='blog-input'
              ...
              <BlogInputs>
                <label className='blog-label'>Portada del blog</label>
                <input
                  className='blog-input'
                  type='file'
            ...
            <div className='blog-row'>
              <BlogInputs>
                <label className='blog-label'>
                  Ruta del blog (Solo usar textos sin acento y separar con "-",
                  evitar usar espacios o los siguientes caracteres:
                  @#$%^&*?``"",./)
                </label>
                <input
                  className='blog-input'
            ...
            <div className='blog-row'>
              <BlogInputs>
                <label className='blog-label'>Link de Video ( Opcional )</label>
                <input
                  className='blog-input'
            ...
            {blog.subTopic?.map((topic: any, index: number) => {
              return (
                <div className='blog-column' key={'topic ' + index}>
                  ...
                  <div className='blog-row'>
                    <BlogInputs>
                      <label className='blog-label'>Título {index + 1}</label>
                      <input
                        className='blog-input'
                        placeholder='Título del Blog'
                        defaultValue={topic.title}
                        onChange={(e: any) => {
                          changeTopicTitle(topic, index, e.target.value);
                        }}
                      />
                    </BlogInputs>
                    <BlogInputs>
                      ...
                      <input
                        ...
                        type='file'
                        onChange={(e) => {
                          changeTopicImage(topic, e.target.files, index);
                        }}
                      />
                    </BlogInputs>
                  </div>
                  <BlogInputs style={{ marginTop: 20 }}>
                    <label className='blog-label'>Texto {index + 1}</label>
                    <ReactQuill
