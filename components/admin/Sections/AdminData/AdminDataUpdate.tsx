    ...
    <UserContain>
      ...
      <>
        ...
        <GridInfoContainer>
          ...
          <InfoResponsive>
            Correo electrónico
            <Label style={{ overflowWrap: 'break-word' }}>{admin.email}</Label>
          </InfoResponsive>
          <InfoResponsive>
            Fecha de Creación
            {admin.created_at && <Label>{formatDate(admin.created_at)}</Label>}
          </InfoResponsive>
          <InfoResponsive>
            Suscripción Actual
            {(admin.level > 0 || admin.final_date > today) && (
              <Label>Gonvar Plus</Label>
            )}
            {admin.level === 0 && <Label>Sin Suscripción</Label>}
          </InfoResponsive>
          <InfoResponsive>
            Teléfono
            <Label>
              ...
              <IconRoleContain>
                <SelectContain key={1}>
                  ...
                  >
                    {!updatedRole && (
                      <>
                        {'superAdmin' === admin.role ? 'superAdmin' : 'admin'}
                      </>
                    )}
                    {updatedRole && value}
                    <CaretD2 />
                    ...
                    <OptionRoleContain>
                      <OptionRole
                        onClick={() => {
                          if (
                            confirm(
                              '¿Seguro que desea actualizar este usuario a superAdmin?',
                            )
                          )
                            updateRole();
                        }}
