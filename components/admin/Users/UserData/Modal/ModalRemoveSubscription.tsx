...
const ModalRemoveSubscription = ({ show, setShow, user }: Props) => {
  ...
  return (
    <Modal show={show} onHide={handleClose} centered>
      <Container>
        <TitleContain>
          <Title>¿Esta seguro que desea quitar la suscripción del usuario <strong>{user.name}</strong>?</Title>
          <CloseIcon onClick={handleClose} />
        </TitleContain>
        <div>
          <p style={{ margin: '0', marginBottom: '5px', fontSize: '20px', fontWeight: '500' }}>Datos:</p>
