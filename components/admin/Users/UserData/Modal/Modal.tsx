...
import {
  addCourseMembershipApi,
  updateCourseMembershipApi,
} from '../../../../api/admin';
...
const Modal1 = ({
  ...
  const addCourse = () => {
    if (!days) {
      ...
      courseForUpdate = tempUserCourse.filter(
        (userCourse: any) => userCourse.course_id === course.id,
      );
      ...
      if (courseForUpdate.length > 0) {
        ...
        updateCourseMembershipApi(body).then((res) => {
          alert(
            'Se agregaron: ' + days + ' días, para el curso: ' + course.title,
          );
          handleClose();
    ...
    if (!days) {
      ...
      if (courseForUpdate.length > 0) {
        updateCourseMembershipApi(courseForUpdate[0]).then((res) => {
          alert(
            'Se eliminaron: ' + days + ' días, para el curso: ' + course.title,
          );
          handleClose();
          openUserCardData(user);
        });
    ...
    <Modal show={show} onHide={handleClose} centered>
      <Container>
        ...
        <Data>
          ...
          <Info>
            <Text1>Correo electrónico</Text1>
            <Text2>{user.email}</Text2>
          </Info>
        </Data>
        <InputContain>
        ...
        {Object.values(course).length > 0 && (
          <>
            ...
            <InputContain>
              ...
              <Input
                type='number'
                placeholder='Número de días activo'
                onChange={(e: any) => {
            ...
            <ButtonContain>
              <PurpleButton onClick={addCourse}>
                Agregar Dias al Curso
              </PurpleButton>
            </ButtonContain>
            {user.user_courses.filter((x: any) => x.course_id === course.id)
              .length > 0 &&
              user.user_courses.filter((x: any) => x.course_id === course.id)[0]
                .final_date > today && (
                <ButtonContain>
                  <PurpleButton onClick={deleteCourse}>
                    Eliminar Dias al Curso
                  </PurpleButton>
                </ButtonContain>
              )}
          </>
