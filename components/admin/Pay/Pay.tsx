...
import {
  generateGetInvoicesCountQuery,
  generateGetInvoicesQuery,
  getDistinctMethodsOfInvoice,
} from './Queries';
...
const Pay = () => {
  ...
  const retrieveInvoices = async () => {
    ...
    try {
      const { invoices, count } = (
        await getInvoicesWithOffsetTestApi({ offset })
      ).data;
  ...
  const newRetrieveInvoices = async () => {
    ...
    try {
      // Inovices count
      const queryInvoicesCount = generateGetInvoicesCountQuery(
        email,
        methods[selectedMethodIndex],
      );
      const responseInvoicesCount =
        await getGenericQueryResponse(queryInvoicesCount);
      const count = responseInvoicesCount.data.data[0]['count'];
      setCount(count);
      // Invoices data
      const queryInvoices = generateGetInvoicesQuery(
        email,
        methods[selectedMethodIndex],
        offset,
      );
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    <option key={`method_${0}`} value='-1'>
                      Sin especificar
                    </option>
                    {methods.map((method, index) => {
                      return (
                        <option key={`method_${index + 1}`} value={index}>
                          {method}
                        </option>
                      );
                    })}
