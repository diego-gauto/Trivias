...
import { AiFillCaretLeft, AiFillCaretRight, AiOutlineMinus, AiOutlinePlus } from 'react-icons/ai';
...
const Cancel = () => {
  ...
  const filterStart = (tempPage: number, dates: any) => {
    setLoader(false);
    getCancelReviewApi(usersPerPage, tempPage * 100, dateFilter, FormatDateForBack(dates[0][0]), FormatDateForBack(dates[0][1])).then((res) => {
      setReviews(res.data);
  ...
  const Gonvar: any = async (dates: any) => {
    let sendData: any = [];
    await cancelReviewExcel(dateFilter, FormatDateForBack(dates[0][0]), FormatDateForBack(dates[0][1])).then(async (res) => {
      console.log(res);
    ...
    <CancelReview>
      <div className='user-answers'>
        <Container>
          <TitleContain className='cancel-filter'>
            ...
            <div className='right-data'>
              <div className='input-contain'>
                ...
                <select defaultValue="all" onChange={(e) => setDateFilter(e.target.value)}>
              ...
              {
                dateFilter === "select" &&
                <div className='calendar-contain' style={hideCalendar ? { borderRadius: "100px", width: 350 } : {}}>
                  <div className='close-tab'>
                    {
                      !hideCalendar
                        ? <AiOutlineMinus className='close' onClick={() => setHideCalendar(true)} />
                        : <AiOutlinePlus className='close' onClick={() => setHideCalendar(false)} />
                    }
          ...
          <div className="pages">
            <div className="index">
              <AiFillCaretLeft className="arrows" onClick={() => { getNextData("backward") }} />
              <p className="default-number" onClick={() => { getNextData("first") }}>1</p>
              <p className="current-number">{currentPage + 1}</p>
              <p className="default-number" onClick={() => { getNextData("last") }}>{maxPages}</p>
              <AiFillCaretRight className="arrows" onClick={() => { getNextData("forward") }} />
            </div>
