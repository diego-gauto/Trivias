...
const ProductsSection = (props: IProductsSectionProps) => {
  ...
  const gerProductElement = (
    ...
    return (
      <GridItem key={'product_landing_' + i}>
        <InputsResponsive>
          <EditText>Producto {i + 1}</EditText>
          <EditInputResponsive
        ...
        <InputsResponsive>
          <EditText>Producto Disponible</EditText>
          <select
            onChange={(e) => {
              updateProductState(e, 'available', i);
            }}
            defaultValue={available === 1 ? 'disponible' : 'agotado'}
          >
        ...
        <InputsResponsive>
          <EditText>Página del Producto</EditText>
          <EditInputResponsive
        ...
        <InputsResponsive>
          <EditText>Imagen del Producto</EditText>
          <FolderInputResponsive
            onChange={(e) => {
              updateProductState(e, 'file', i);
              getImage(e.target.files, i);
            }}
            type='file'
  ...
  const elementsToShow = [...chunk1, ...chunk2].map(
    (product: Product, i: number) => {
      return gerProductElement(product, i);
    },
  );
  const onSave = async () => {
    productsData.map(async (product, index) => {
      if (product.new_image) {
        await updateLandingProductImage(product.new_image, product.id).then(
          (url) => {
            product.image = url;
          },
        );
  ...
  return (
    <ProfileData style={{ boxShadow: 'none', background: 'none' }}>
      <GridContainer>{elementsToShow}</GridContainer>
      <EditButtons>
