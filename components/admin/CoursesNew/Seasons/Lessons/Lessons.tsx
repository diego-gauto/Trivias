...
import {
  ...
  getLessonFromApi,
  updateLessonFromApi,
  updateLessonImageFromApi,
  updateLessonMaterialContect,
} from '../../../../api/courses';
import ReactPlayer from 'react-player';
import {
  updateLessonHomeWorks,
  updateLessonImage,
...
const Lessons = () => {
  ...
  const createLesson = async () => {
    ...
    if (!checkErrors) {
      ...
      await createLessonFromApi(lesson)
        .then(async (res) => {
          lesson.id = res.data;
          await updateLessonImage(courseID, seasonID, tempImage, res.data).then(
            (banner) => {
  ...
  const updateLesson = async () => {
    ...
    if (!checkErrors) {
      ...
      lesson.extraMaterial = extraMaterial;
      await Promise.all(
        lesson.extraMaterial.map(async (mat: any) => {
          if (!mat.id) {
            await updateLessonHomeWorks(
              courseID,
              seasonID,
  ...
  const deleteLesson = () => {
    ...
    if (
      confirm(
        `¿Desea eliminar la leccion ${lesson.name}?, esta accion no tiene marcha atras`,
      )
    ) {
      lesson.quizzes = quiz;
      lesson.lesson_homeworks = homeWorkData;
    ...
    <AdminContain style={{ flexDirection: 'column' }}>
      ...
      <div className='courses-header'>
        <h1 className='main-title'>
          {!updateMode ? 'Agregar Lección' : 'Editar Lección: ' + title}
        </h1>
        {updateMode && (
        ...
        <div className='lesson-contain'>
          <div className='rows' style={{ justifyContent: 'flex-end' }}>
            <div className='input-contain' style={{ width: 'unset' }}>
              {!loader ? (
                <>
                  {!updateMode ? (
                    <button className='save-button' onClick={createLesson}>
                      Guardar
                    </button>
                  ) : (
                    <button className='save-button' onClick={updateLesson}>
                      Editar
                    </button>
                  )}
                </>
          ...
          <div className='rows'>
            <div className='input-contain'>
              <label className='input-label'>Título de la Lección</label>
              <input
            ...
            <div className='input-contain'>
              <label className='input-label'>Número de Lección</label>
              <input
                type='number'
          ...
          <div className='rows'>
            <div className='input-contain'>
              <label className='input-label'>Hipervínculo del video</label>
              <input
            ...
            <div className='input-contain'>
              <label className='input-label'>Puntos Acreditados</label>
              <input
                type='number'
                placeholder='100'
                className='input-create'
            ...
            <div className='input-contain'>
              <label className='input-label'>Portada de la Lección</label>
              <input
                type='file'
                className='input-create'
          ...
          <div className='rows'>
            ...
            <div className='input-contain'>
              <label className='input-label'>Material Adicional</label>
              <input
                type='file'
                className='input-create'
                onChange={(e) => {
                  getExtraMaterial(e.target.files);
                }}
              />
                        ...
                        {extra.id ? (
                          <div className='material-content'>
                            <Link href={extra.material}>
                              <a target='_blank' className='extra-hmk'>
                                Tarea {index + 1}
                              </a>
                            </Link>
                            <input
                          ...
                          <div className='material-content'>
                            <a target='_blank' className='extra-no-hmk'>
                              Tarea {index + 1}
                            </a>
                            <input
                              placeholder={'Titulo de la tarea ' + (index + 1)}
          ...
          <div className='rows'>
            <div className='input-contain'>
              <label className='input-label'>Tarea o Quiz</label>
            ...
            <div className='rows'>
              <div className='toggle'>
                <div
                  ...
                  style={{
                    background: lesson.homework ? '#6717cd' : 'none',
                    color: lesson.homework ? '#fff' : '#6717cd',
                  }}
                >
                  Tarea
                </div>
                <div
                  ...
                  style={{
                    background: lesson.quiz ? '#6717cd' : 'none',
                    color: lesson.quiz ? '#fff' : '#6717cd',
                  }}
                >
                  Quiz
                </div>
          ...
          adipiscing morbi mattis eget.'
                    id='quill'
                    theme='snow'
                    className='quill-lesson'
                    formats={formats}
                    modules={modules}
              ...
              <div className='rows'>
                <div className='input-contain'>
                  <label className='input-label'>Puntos de la Tarea</label>
                  <input
                    placeholder='50'
                    className='input-create'
                    type='number'
          ...
          {lesson.quiz && (
            <>
              <div className='rows'>
                <div className='input-contain'>
                  <label className='input-label'>Nombre del Quiz</label>
                  <input
            ...
            adipiscing morbi mattis eget.'
                      ...
                      {quest.answers.length > 0 && (
                        <div className='answer-contain'>
                          <p className='answer-text'>Respuestas</p>
                          {quest.answers.map((ans: IAnswer, ind: number) => {
                            return (
                              <div
                                className='all-answers'
                                key={'quest_ans' + index + ind}
                              >
                                <div
                                  style={{
                                    backgroundColor: ans.status
                                      ? '#00d14d'
                                      : '#D10000',
                                  }}
                                  className='circle'
