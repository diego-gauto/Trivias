...
const Seasons = () => {
  ...
  const deleteSeaons = (seasonData: ISeason) => {
    ...
    if (
      confirm(
        `¿Desea eliminar temporada ${seasonData.name}?, esto eliminara todas las lecciones`,
      )
    ) {
          ...
          <div className='courses-header'>
            ...
            <div className='courses-buttons'>
              <button onClick={addSeason}>
                Agregar Temporada <AiOutlinePlus style={{ fontSize: 20 }} />
              </button>
            </div>
          </div>
          <SeasonContainer>
                ...
                <div className='season-contain' key={'Seasons_' + index}>
                  <div className='title-contain'>
                    {editSeasonName === index ? (
                      <div className='title-complete-contain'>
                        ...
                        <button
                          ...
                          onClick={() => saveData(seasonData, index)}
                        >
                          Guardar
                        </button>
                        <AiFillEdit
                          className='edit-icon'
                          onClick={() => {
                            startSeasonUpdate(index);
                          }}
                        />
                      </div>
                    ) : (
                      <div className='title-complete-contain'>
                        <p className='season-title'>{seasonData.name}</p>
                        <AiFillEdit
                          className='edit-icon'
                          onClick={() => {
                            startSeasonUpdate(index);
                    ...
                    <div className='arrow-button'>
                      <button
                        ...
                        onClick={() => addLesson(seasonData.id)}
                      >
                        Añadir lección
                  ...
                  {openSeason === index && seasonData.lessons?.length > 0 && (
                    <div className='lesson-content'>
                      {seasonData.lessons.map(
                        (lesson: ILesson, ind: number) => {
                          return (
                            <div
                              className='lesson-contain'
                              key={'LessonShow_' + ind}
                            >
                              ...
                              <div className='lesson-data'>
                                ...
                                <p
                                  ...
                                  onClick={() => {
                                    editLesson(seasonData.id, lesson.id);
                                  }}
                                >
                                  Editar Leccion
                                </p>
