...
import {
  LESSON_PATH,
  NAILS_FORM,
  NAILS_LANDING_REDIRECT,
  NAILS_REVOLUTION_REDIRECT,
  PLAN_PATH,
  PREVIEW_PATH,
  PURCHASE_PATH,
  SIGNUP_PATH,
...
export const CourseModule = (props: ICourseModuleProps) => {
  ...
  const doVideoStuff = () => {
    ...
    if (num === 1) {
      ...
      videoSrc =
        'https://video.gonvar.io/media/nails_master_revolution/landing/master.m3u8';
    }
    if (num === 2) {
      ...
      <Row>
        <Col sm={12} md={5} className='first-col'>
          ...
          <Row className='level'>
            ...
            <Text03 style={{ padding: 0 }} level={data.difficulty}>
        ...
        <Col sm={12} md={5} className='second-col'>
          {data.id && (
            <PurpleButton
              text={responsive768 ? 'Comprar' : 'Comenzar ahora'}
              onClick={() => {
                goTo();
              }}
            />
