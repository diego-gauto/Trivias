...
const Pagination = (props: IPagination) => {
  ...
  return (
    <PaginationContainer>
      ...
      <div className="index">
        <HiArrowCircleLeft className="arrows" onClick={() => switchPage("back")} />
        <p className="default-number" onClick={() => switchPage("first")}>1</p>
        <p className="current-number">{currentPage + 1}</p>
        <p className="default-number" onClick={() => switchPage("last")}>{totalPage}</p>
        <HiArrowCircleRight className="arrows" onClick={() => switchPage("next")} />
      </div>
    </PaginationContainer>
  )
}
